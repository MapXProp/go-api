		...
		err = tx.QueryRowContext(ctx, `
			...
			ON CONFLICT (user_id, submission_key) WHERE submission_key IS NOT NULL DO UPDATE SET
				...
				custom_project_name_en = CASE
					...
					ELSE ''
				END,
				custom_project_name_language = CASE
					WHEN listings.custom_project_name IS NOT DISTINCT FROM EXCLUDED.custom_project_name THEN listings.custom_project_name_language
					ELSE 'en'
				END,
